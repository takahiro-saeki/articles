...
const batchSummary=batches.map(batch=>{const group=items.filter(x=>x.batch===batch);return `- バッチ${String(batch).padStart(2,'0')}: ${group.map(x=>x.id).join('、')}。完成 ${group.filter(x=>x.status==='完成').length}/${group.length}件。`;}).join('\n');
